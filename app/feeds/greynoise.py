...
class GrayNoiseConnector:
    ...
    def check_ip(self, ip_address: str) -> Tuple[Dict[str, Any], bool]:
        ...
        try:
            ...
            endpoint = f"{self.API_BASE}/ips/{ip_address}" if not self.use_community else f"{self.COMMUNITY_API_BASE}/ips/{ip_address}"
            ...
            if "rate_limit_remaining" in response.headers:
                self.stats.api_calls_remaining = int(response.headers.get("rate_limit_remaining", 0))
            
            logger.debug(f"GrayNoise IP check for {ip_address}: {data.get('classification', 'unknown')}")
            return data, True
    ...
    def _normalize_ips(self, ip_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for item in ip_data:
            ...
            normalized_item = {
                ...
                "first_seen": self.last_query_time.isoformat() if self.last_query_time else None,
    ...
    async def query_ips_async(self, ips: List[str]) -> Tuple[List[Dict[str, Any]], bool]:
    ...
    async def get_trending_ips_async(self, limit: int = 100) -> Tuple[List[Dict[str, Any]], bool]:
...
async def poll_greynoise_feed(api_key: str = "", use_community: bool = False, database=None):
    """Async function to poll GrayNoise feed (for scheduler integration)"""
    connector = get_greynoise_connector(api_key=api_key, use_community=use_community, database=database)
